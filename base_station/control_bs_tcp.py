...
class swarm(object):
    ...
    def mesh_check(self):
        if time.time()-self.sniff_timer>2:
            if not self.mesh_adjusting:               #if mesh is adjusting , pie's not supposed to sample signal 
                ...
                if pkg[0].haslayer(RadioTap):
                    if type(pkg[0].dBm_AntSignal)==int:
                        #print("dBm="+str(pkg[0].dBm_AntSignal))
                        if len(self.sniff_history)<10:
                            self.sniff_history.append(pkg[0].dBm_AntSignal)
                            return
                        else:
                            self.sniff_history[self.sniff_counter%len(self.sniff_history)]=pkg[0].dBm_AntSignal
                            self.sniff_counter+=1
                            sum=0
                            for i in range(0,len(self.sniff_history)):
                                sum+=self.sniff_history[i]
                            sum=sum/len(self.sniff_history)
                            print("dBm avg is="+str(sum))
                            if sum>-20:
...
class Drone(object):
    ...
    def __init__(self,id,stream=False):
        ...
        #os.system("ssh bat"+str(id+1)+"@"+pi_ip+" nc -l -p"+PIE_PORT)
        self.control_socket=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        self.control_socket.bind((BS_IP,PIE_PORT))
        print("bind done")
        self.control_socket.connect((pi_ip,PIE_PORT))
        print("connected")
        os.system("ssh bat"+str(id+1)+"@"+pi_ip+" source tcp_routing")
        self.command_queue=[]
...
def main():
    ...
    if stream:
        #ssh bat1@192.168.2.10 ffmpeg -i udp:192.168.2.10:11111 -r 15 -f rtp rtp:192.168.2.1:11111
